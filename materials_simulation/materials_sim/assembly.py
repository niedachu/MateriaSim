"""Strict molecular packing contracts and audited explicit-GAFF topology assembly."""

import math
import re

from .files import sha256, write_json
from .prebuilt import declared_counts, molecular_definitions
from .protocol import mdp_values
from .schema import fields, identifier, integer, number
from .topology import gro_atoms, section_rows
...
def validate_packing(spec, sources):
    ...
    counts, seen = dict.fromkeys(declared_counts(spec), 0), set()
    for group in groups:
        ...
        for lower, upper, length in zip(group["min_nm"], group["max_nm"], scenario["box_nm"]):
    ...
    if counts != declared_counts(spec):
