...
from .files import inventory, run_lock, utc_now, write_json
from .gromacs import engine_info
from .gromacs_build import prepare_system
from .schema import load_spec
from .state import new_attempt, set_status, source_identity


def build(spec_path, run_root, candidate="gmx", packmol_candidate="packmol"):
    """Create a unique v2 Run using explicit native engine/packer candidates; v1 is read-only."""
    ...
    root = run_root / (spec["id"] + "-" + uuid.uuid4().hex)
