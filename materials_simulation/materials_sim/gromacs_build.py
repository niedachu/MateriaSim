...
from .files import content_hash, inventory, sha256, write_json
from .gromacs import command
from .gromacs_stage import compile_stage
from .protocol import derive_mdp
from .prebuilt import declared_counts, structure_assets, validate_prebuilt_sources, verify_processed_models
from .topology import atom_mapping, gro_atoms, molecule_counts
...
def snapshot(root, spec, sources, engine):
    ...
    if spec["scenario"]["kind"] == "packed_liquid":
        from .assembly import write_topology
...
def prepare_system(root, spec, sources, engine, attempt, packmol_candidate="packmol"):
    ...
    if spec["scenario"]["kind"] == "packed_liquid":
        from .gromacs_packed import prepare_packed
