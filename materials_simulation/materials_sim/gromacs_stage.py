...
from .files import inventory, read_json, sha256, write_json
from .gromacs import checkpoint, command
from .protocol import mdp_values
from .topology import gro_atoms
...
def execute_stage(root, stage, engine, attempt, threads, remaining):
    ...
    result = command(engine, args, folder, attempt / f"run-{name}", root / "inputs", seconds=remaining + 15)
