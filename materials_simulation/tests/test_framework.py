...
from materials_sim.gromacs_build import check_includes
from materials_sim.gromacs_stage import assess_stage, check_numerics, execute_stage
from materials_sim.files import contained, read_json, run_lock, sha256, verify_hashes, write_json
from materials_sim.protocol import derive_mdp
from materials_sim.schema import integer, load_spec, validate_analysis
from materials_sim.state import source_identity
from materials_sim.topology import atom_mapping
...
class FrameworkTests(unittest.TestCase):
    ...
    def test_lock_collision(self):
        ...
        script = "from materials_sim.files import run_lock\nimport sys\nwith run_lock(sys.argv[1]): print('locked')"
    ...
    def test_interrupted_checkpoint_does_not_require_final_gro(self):
        ...
        with patch("materials_sim.gromacs_stage.checkpoint", return_value=state):
            self.assertFalse(assess_stage(self.root, stage, {})["complete"])
        with patch("materials_sim.gromacs_stage.checkpoint", return_value=dict(state, step=1000, time_ps=2)):
