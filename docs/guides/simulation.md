...
| `engines/gromacs/stage.py` / `engines/gromacs/command.py` | GROMACS 编译、执行、完成检查与原生检查点 |
...
当前可安装核心为 `src/materiasim/`，共享资产在 `catalog/`，研究编排见[研究指南](research.md)。A 批历史验收保持原记录，B—D 本轮状态见[架构方案](../plans/2026-09-15__architecture__simulation-package-and-research-bundles__implementation-plan.md)。CPU/GPU 共用执行配置尚未实施。
